  ...
  try {
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 2000,
        system: `Tu es un assistant agenda concerts en Belgique. Tu consultes les agendas officiels des salles de concert belges. Retourne UNIQUEMENT un tableau JSON brut sans markdown, sans texte avant ni après. Format strict: [{"date":"YYYY-MM-DD","artist":"Nom","venue":"Salle","city":"Ville","genre":"Rock|Folk / Alt|Classique|Opéra","ticketUrl":"https://..."}]. Genres autorisés exactement: Rock, Folk / Alt, Classique, Opéra. Pour ticketUrl, mets OBLIGATOIREMENT une URL : soit le lien direct de billetterie du concert (Ticketmaster, salle, etc.), soit l'URL de l'agenda/billetterie de la salle si pas de lien direct. Ne mets jamais null. Si aucun concert trouvé, retourne [].`,
